...
def test_time_based(mock_log_data):
    ...
    assert features['within_day_mean'] == pytest.approx(41330.543183909555, rel=1e-2)
    assert features['within_day_median'] == pytest.approx(37800.0, rel=1e-2)
    assert features['within_day_mode'] == pytest.approx(21600.0, rel=1e-2)
    ...
    assert features['within_day_q1'] == pytest.approx(23113.75, rel=1e-2)
    assert features['within_day_q3'] == pytest.approx(57600.0, rel=1e-2)
    assert features['within_day_iqr'] == pytest.approx(34486.25, rel=1e-2)
    assert features['within_day_geometric_mean'] == pytest.approx(35069.233548115764, rel=1e-2)
    assert features['within_day_geometric_std'] == pytest.approx(1.9726454507370417, rel=1e-2)
    assert features['within_day_harmonic_mean'] == pytest.approx(0.0, rel=1e-2)
    assert features['within_day_skewness'] == pytest.approx(0.3603519661740256, rel=1e-2)
    assert features['within_day_kurtosis'] == pytest.approx(-0.9142275965359778, rel=1e-2)
    assert features['within_day_coefficient_variation'] == pytest.approx(0.49820042247168106, rel=1e-2)
    assert features['within_day_entropy'] == pytest.approx(9.501009299480838, rel=1e-2)
    assert features['within_day_skewness_hist'] == pytest.approx(1.7511033515349685, rel=1e-2)
    assert features['within_day_kurtosis_hist'] == pytest.approx(2.611589422813226, rel=1e-2)
